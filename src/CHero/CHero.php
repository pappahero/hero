 ...
 * @package LydiaCore
 */
class CHero implements ISingleton {
	...
	protected function __construct() {
		...
		// include the site specific config.php and create a ref to $he to be used by config.php
		$he = &$this;
		require(HERO_SITE_PATH.'/config.php');
  ...
  /**
	 ...
	 * @return CHero The instance of this class.
	 */
	public static function Instance() {
		if(self::$instance == null) {
			self::$instance = new CHero();
		}
  ...
  public function ThemeEngineRender() {
    ...
    // Förstår inte vad som lagras i sessionen här och varför. Vad är det som ligger
    // i sessions data-array som behöver sparas.
  
    // Is theme enabled?
    if(!isset($this->config['theme'])) { return; }
    
    // Get the paths and settings for the theme, look in the site dir first
    $themePath 	= HERO_INSTALL_PATH . '/' . $this->config['theme']['path'];
    ...
    if(isset($this->config['theme']['parent'])) {
      $parentPath = HERO_INSTALL_PATH . '/' . $this->config['theme']['parent'];
      $parentUrl	= $this->request->base_url . $this->config['theme']['parent'];
    }
    
    // Add stylesheet name to the $he->data array
    $this->data['stylesheet'] = $this->config['theme']['stylesheet'];
    
    // Make the theme urls available as part of $he
    ...
    // Detta är metoden är rita upp menyn med. Ganska komplex. Måste kolla mer
    // på den. Fast jag är tveksam om den ritar ut. Kanske är det bara mappning
    // som sker

    // Include the global functions.php and the functions.php that are part 
    // of the theme
    
    $he = &$this;
    // Vad denna gör här övergår mitt förstånd. Har det med tillgång till variabler
    // i vyerna att göra liksom det fungerar för config. Kanske tillgång till 
    // CHeros varibler i /themes/functions snarare
    
    // First the default Lydia themes/functions.php
    include(HERO_INSTALL_PATH . '/themes/functions.php');
    ...
    // Extract $he->data to own variables and handover to the template file
    extract($this->data);  // OBSOLETE, use $this->views->GetData() to set variables
    extract($this->views->GetData());
    // Förstår inte riktigt heller hur detta fungerar men det verkar vara så att 
    // variablerna som finns i CViewContainers data-array returneras och extraheras
    // här.
    if(isset($this->config['theme']['data'])) {
      extract($this->config['theme']['data']);
      // Detta är data-arrayen som finns i config-filen. Det måste betyda att det är
      // en eller två andra data-arrayer som extraheras här innan.
    }
  ...
  /**
   * Draw HTML for a menu defined in $he->config['menus'].
   *
